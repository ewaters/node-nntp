...
NNTP.prototype.overview = function (what, cb) {
	...
	if (typeof what === 'object') {
		options = what;
		if (options.range) {
			what = options.range.join('-');
		}
		else {
			return cb("Must pass 'range' or 'id' in the options object");
		}
	}
	var command = 'XOVER';
	...
	this._send(command, what, function (err, code, r, type) {
		self._bufferEnc = undefined;
		if (err) return cb(err);

		// Ignore the first line of status
		var buffer = self._buffer.replace(/^.+?\r\n/, '');

		if (options.raw) return cb(null, buffer.trim());

		var list = buffer.split(reCRLF);

		var data = list.map(function (item) {
			return item.split(reTAB);
		});
...
NNTP.prototype._send = function(cmd, params, cb) {
  ...
  if (!this._curReq && this._queue.length) {
	...
	var out = this._curReq.cmd;
    if (this._curReq.params !== undefined) {
		out += ' ' + this._curReq.params;
    }
	this.emit('debug_output', out);
    this._socket.write(out + bytesCRLF);
  }
};
